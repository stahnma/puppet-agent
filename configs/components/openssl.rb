component "openssl" do |pkg, settings, platform|
  ...
  if platform.is_cross_compiled_linux?
    ...
    unless (platform.is_fedora? && platform.os_version.delete('f').to_i >= 26) or platform !~ /(debian-9|ubuntu-18.04)/
      pkg.build_requires 'pl-binutils'
    end
    pkg.build_requires 'pl-gcc' unless platform.name =~ /(debian-9|ubuntu-18.04)/

    if platform.name =~ /(debian-[\d]|ubuntu-18.04)/
